...
//! `typed-marine-hold` uses [`TypedMarineHold`] (`Lab::attach_undock` /
//! `attach_marine_hold` — NED pose, not StationKeep).
...
fn main() {
    ...
    let run = match kind {
        ...
        Kind::TypedMarineHold => lab.research(&mut TypedMarineHold::default(), 0.02, 40),
