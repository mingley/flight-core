...
- typestate methods generated by `impl_aerial_offboard_now!` (`admit_offboard_now`, `set_velocity_now` / `set_position_now` / `hold_now`, async `set_velocity` / `set_position` / `hold`, `apply_velocity_command_now`; `COMMANDS` is `AERIAL_OFFBOARD_COMMANDS`)
