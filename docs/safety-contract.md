...
- fault-injection list (`REVOKE_ON` / `inject` / `FAULTS` / `run_revoke_table` leftover Offboard `COMMANDS`, PX4 `inject_revoke` / `run_px4_revoke_table` / `run_px4_gps_loss` / `flight-test-px4`, ArduPilot `inject_revoke` / `run_ardupilot_revoke_table` / `flight-test-ardupilot`, HITL `leftover_after_deadline_miss` / `run_hitl_revoke_table` / `flight-test-hitl`, ROS 2 `leftover_after_failsafe` / `run_ros2_revoke_table` / `flight-test-ros2`, [`docs/generated/aerial-offboard.faults.md`](generated/aerial-offboard.faults.md) / [`aerial-offboard.faults.txt`](generated/aerial-offboard.faults.txt))
...
GPS-loss (`EstimatorInvalid` + `GPS_LOSS.require`) is `run_px4_gps_loss` /
