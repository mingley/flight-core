...
- fault-injection list (`REVOKE_ON` / `inject` / `FAULTS` / `run_revoke_table` leftover Offboard `COMMANDS`, PX4 `inject_revoke` / `run_px4_revoke_table` / `run_px4_gps_loss` / `flight-test-px4`, ArduPilot `inject_revoke` / `run_ardupilot_revoke_table` / `run_ardupilot_gps_loss` / `flight-test-ardupilot`, HITL `leftover_after_deadline_miss` / `run_hitl_revoke_table` / `run_hitl_gps_loss` / `flight-test-hitl`, ROS 2 `leftover_after_failsafe` / `run_ros2_revoke_table` / `run_ros2_gps_loss` / `flight-test-ros2`, [`docs/generated/aerial-offboard.faults.md`](generated/aerial-offboard.faults.md) / [`aerial-offboard.faults.txt`](generated/aerial-offboard.faults.txt))
...
GPS-loss (`EstimatorInvalid` + `AerialOffboard::GPS_LOSS_REQUIRE`) is
`run_px4_gps_loss` /
...
is `WorldRack::run_hitl_revoke_table`; leftover GPS-loss is
`WorldRack::run_hitl_gps_loss` /
...
`leftover_after_disarm` / `run_ros2_revoke_table`; leftover GPS-loss is
`run_ros2_gps_loss` / `cargo run -p flight-ros2 --bin flight-test-ros2`.
