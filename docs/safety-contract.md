...
| FC-INV-002 | `ActuationPermit.epoch == backend.epoch` | non-`Clone` permit | `permit.check` at the backend boundary; reconnect / `begin_session` bumps epoch; leftover Armed cannot `set_motor_thrust_now`; PX4/ArduPilot/`NullBackend`/`SimBackend`/`WorldBackend` refuse offboard/climb/actuators/thrust after `actuation_revoked` |
