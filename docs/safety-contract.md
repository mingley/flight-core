...
- typestate methods generated by `with_aerial_offboard_commands!` → `impl_aerial_offboard_now!` (`admit_offboard_now`, each kernel command's `*_now` / async wrapper, `for_each_offboard_now`, `apply_velocity_command_now`; `COMMANDS` is `AERIAL_OFFBOARD_COMMANDS`)
...
- fault-injection list (`REVOKE_ON` / `inject` / `FAULTS` / `run_revoke_table` leftover Offboard `COMMANDS`, [`docs/generated/aerial-offboard.faults.md`](generated/aerial-offboard.faults.md) / [`aerial-offboard.faults.txt`](generated/aerial-offboard.faults.txt))
