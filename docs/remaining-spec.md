...
- Single-source aerial authority table in `safety` (`define_aerial_authority!`): kernel revoke predicate, `admit_offboard_command`, `AERIAL_OFFBOARD_COMMANDS`, heartbeat/command bounds, Creusot `ensures`, capability diagrams, `ContractEdge` transition table. `vehicle_contract! { from_kernel }` aliases it (`AerialOffboard::admit`, `COMMANDS`). `impl_aerial_offboard_now!` generates the OffboardControl now-methods. `AerialOffboard::evaluate` runs the generated `MONITORS` (including `OffboardAdmitted`). `prove_aerial_authority!` is the Kani harness. Native ULog subset plus JSONL share `evaluate_trace`. `run_revoke_table` / `Scenario::HITL_MISS` / `WorldRack::contract_deadline_miss` / `differential_gps_loss` evaluate the same `Requirement`s plus capability monitors.
