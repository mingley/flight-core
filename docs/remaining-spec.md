...
- Consume-self typestate for aerial / ground / marine vehicles (`Vehicle`, `GroundVehicle`, `MarineVehicle`) with compile-fail UI tests under `crates/flight-core/tests/ui/` (131 `.rs` files).
- Revocable `ActuationPermit` (non-`Clone`) plus backend `authority_epoch`. Failsafe / disarm / disconnect / stale heartbeat / estimator / IMU events increment the plant epoch. Stale permits cannot setpoint even when the Rust typestate is still `Offboard` / `Moving` / `Underway`. Mode-change now-APIs (`enter_offboard_now`, `start_takeoff_now`, `land`) check the live permit before the backend. A leftover `Vehicle<Armed>` after an async PX4 disarm HEARTBEAT is `StaleEpoch`. `apply_velocity_command_now` additionally rejects a command older than `COMMAND_MAX_AGE_MS`.
- Single-source aerial authority table in `safety` (`define_aerial_authority!`): kernel revoke predicate, `admit_offboard_command`, heartbeat/command bounds, Creusot `ensures`, capability diagrams, `ContractEdge` transition table. `vehicle_contract! { from_kernel }` aliases it (`AerialOffboard::admit`). `prove_aerial_authority!` is the Kani harness. Native ULog subset plus JSONL share `evaluate_trace`. `run_revoke_table` / `Scenario::HITL_MISS` / `WorldRack::contract_deadline_miss` / `differential_gps_loss` evaluate the same `Requirement`s.
